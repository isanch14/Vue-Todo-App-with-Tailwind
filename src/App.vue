<template>
<div
        class="h-screen w-width bg-purple-600 flex flex-col justify-center items-center"
    >
        <div class="header">
            <h1 class="text-4xl">⚾️ My Todo App with Tailwind and Vue ⚾️</h1>
            <div class="form flex">
                <input
                    v-model="newTodo"
                    @keyup="checkAdd"
            ...
            <div class="completed mt-8" v-if="totalCompleted > 0">
                ...
                <ul id="completedList">
                    <li
                      ...
                      class="text-center p-3 bg-white line-through text-black-500 mt-4 rounded shadow-lg cursor-pointer hover:bg-red-500 hover:text-white"
                    >{{completed}}</li>
                </ul>
            </div>
        </div>
